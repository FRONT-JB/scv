...
| BUILD-010 | pending | source/target 및 안전 계약 확정 | BUILD-000 | 수정된 GOAL·계약 결정 |
...
| BUILD-110 | pending | workspace binding·checkpoint | BUILD-050, BUILD-100 | drift 차단·재결속 |
...
- immutable binding에는 repository/common-dir/worktree/branch identity를 기록하고
  HEAD·content는 checkpoint와 rebind history에 둔다.
- target `.scv/tasks`, runtime과 local manifest만 fingerprint에서 제외한다.
- 정상 backend 변경, 외부 drift, HEAD 변경, 정확 복구와 명시적 재결속을 구분한다.
...
| C08 checkpoint·drift·재결속 | BUILD-110 |
