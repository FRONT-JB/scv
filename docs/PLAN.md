...
| BUILD-020 | done | cross-host plugin package scaffold | BUILD-010 | 설치 가능한 plugin root |
| BUILD-030 | done | 소비 프로젝트 template·Schema·Policy | BUILD-020 | setup payload 계약 |
...
- identity `scv`, version `0.1.0`으로 Codex/Claude manifest를 일치시켰다.
- 두 marketplace가 `./plugins/scv`만 가리키고 host capability 기준선을 선언한다.
- Codex plugin validator와 Claude `plugin validate --strict`의 plugin/marketplace
  검증을 통과했으며 package 외부 링크와 runtime payload는 없다.
...
- versioned Draft 2020-12 schema 17개와 schema별 정상·경계·단일 변형 오염
  fixture를 추가했다.
- 모든 object schema를 closed object로 정의하고 unknown field, version mismatch,
  잘못된 상태·attempt·경로가 거부됨을 validator로 확인했다.
- 정확한 4개 host/Skill 링크, setup 소유 경로·digest, safe resume, finding waiver,
  evidence 성공 조건과 READY gate를 schema에 결속했다.
- request와 handoff는 JSON control data를 권위 원본으로 두고 Markdown을 결정적으로
  렌더링하도록 source/target 계약을 명확히 했다.
- portable project template 10개를 임시 소비 프로젝트에 렌더링했으며 source 저장소에
  `.scv`가 생성되지 않음을 확인했다.
...
- `format` annotation에 의존하지 않고 UUIDv4와 UTC timestamp를 stdlib로 검증한다.
...
- operation ID 중복, role·schema·artifact 참조 불일치와 registry 순서 drift를
  결정적으로 차단한다.
...
- setup manifest의 항목이 exact expected inventory와 일치하고 path가 유일하며,
  종류별 허용 root와 Git 추적 값이 선언된 경계에서 벗어나면 차단한다.
...
- event sequence·ID·previous digest chain과 kernel-only 상태 전이 권한을 검증한다.
...
- step·acceptance ID 유일성, 의존 대상 존재·비순환 DAG와 kernel 보호 경로 제외를
  semantic validator로 확인한다.
...
- current run·step reference, run/step/attempt ID 유일성과 상태별 종료 필드의
  정합성을 검증한다.
...
- accepted risk가 실제 waiver 가능한 finding의 부분집합이고 모든 verification이
  통과했음을 cross-artifact로 검증한다.
...
- authoritative `request.json`·`handoff.json`에서 runtime Markdown을 결정적으로
  렌더링하고, 승인 request/plan과 완료 handoff를 target `.scv/docs/work-items`에
  snapshot으로 발행한다.
