...
기존 실행 구현은 재구성 시작 시점에 제거한다. 이 문서와
[PLAN.md](PLAN.md), [설계 결정](decisions/README.md)이 새 구현의 기준이며 기존
runtime 계약은 호환성 탐지 외에 실행하거나 복사하지 않는다.

## 소스 저장소와 소비 프로젝트

이 저장소는 SCV를 직접 구성한 소비 프로젝트가 아니라 설치 가능한 플러그인을
만드는 소스 저장소다. 권위 있는 배포 원본은 다음 경로에 둔다.

```text
<scv-source>/
├── plugins/scv/                       # self-contained plugin package
...
`plugins/scv/**`는 패키지 밖 경로와 심볼릭 링크에 의존하지 않는 self-contained
배포 단위다. 소스 저장소 루트에는 `.scv`, 프로젝트 Skill 링크 또는 runtime
태스크를 만들지 않는다. setup은 SCV source checkout 자체 또는 source와 포함
관계인 경로를 소비 프로젝트로 받으면 거부한다. setup E2E는 source 밖 임시 소비
프로젝트에서만 실행한다.

setup이 소비 프로젝트에 materialize한 설치 사본의 권위 원본은
`<project>/.scv/plugin/**`이며, 아래의 목표 구조는 모두 소비 프로젝트 기준이다.
계획의 `BUILD-xxx`는 이 저장소의 구현 태스크이고 runtime의 `<task-id>`와 서로
다른 식별자다.
...
- 소비 프로젝트 runtime에서 플러그인과 Skill의 권위 원본은 `.scv/plugin/**`다.
...
| `perform-change.md` | backend | 결속된 현재 worktree에서 sandboxed 구현 |
...
4. 사용자 승인 후 검증된 package를 `.scv/plugin`에 배포하고 Skill별 상대
   심볼릭 링크를 구성한다.
...
Ollama 모델 다운로드, plugin 설치·비활성화 또는 호스트 외부 설정 변경은 setup이
실행하지 않는다. 필요하면 정확한 명령과 영향을 안내하고 사용자가 별도로 수행한
뒤 `validate`로 결과만 확인한다.

non-terminal runtime 태스크가 하나라도 있으면 package, 추적 config 또는 문서를
바꾸는 `upgrade`를 차단한다. 같은 digest를 복원하는 `repair`만 execution lease가
없고 최종 검증 결과가 기존 manifest와 같을 때 허용한다.

프로젝트 Skill 링크 또는 필수 sandbox capability를 지원하지 않는 호스트 버전은
copy fallback 없이 setup을 차단하고 지원 버전으로 업그레이드하도록 안내한다.

### 활성화 모드

- `linked`: 프로젝트의 `.agents/skills`와 `.claude/skills` 링크를 사용하는 유일한
  정상 runtime 모드
- `installed`: marketplace 또는 `--plugin-dir`에서 setup을 bootstrap하고 배포
  패키지를 검증하기 위한 일시 모드

설치형 플러그인의 namespaced `setup`이 소비 프로젝트에 package를 복사하고 링크를
만든다. 새 `.claude/skills` 최상위 디렉터리는 현재 Claude 세션에서 즉시 발견되지
않을 수 있으므로 setup 직후 이어서 진행할 때는 이미 로드된 namespaced
`scv-1-plan`을 사용하고, 다음 세션부터 project link를 사용한다. cutover가 끝나면
설치형 plugin을 비활성화하거나 해당 세션을 종료한다. 다음 실행에서 installed와
linked 사본이 동시에 발견되면 같은 Skill의 deduplication을 가정하지 않고
fail-closed로 차단한다.

설치형 플러그인은 host cache로 복사되므로 필요한 파일을 모두 package 내부에
포함하고 외부 링크에 의존하지 않는다. setup은 package version과 digest를 확인한
뒤 `.scv/plugin`에 복사하며 cache 자체를 runtime 원본으로 사용하지 않는다.
...
- `PLAN_READY`에서 나중에 구현을 시작할 때 workspace checkpoint를 다시 검증한다.
  같은 binding에서 기준 HEAD가 바뀌었으면 재기준 설정과 새 계획 승인을 거쳐야
  한다.
...
- 같은 binding의 workspace를 원래 checkpoint로 복구할 수 없으면 사용자가 명시적
  재기준 설정을 선택할 수 있다. 새 checkpoint를 기록하고 `PLANNING`으로 돌아가
  전제 검토와 재승인을 요구한다. repository, worktree 또는 branch binding 변경은
  재기준 설정 대상이 아니다.
...
checkpoint를 기록한다. 예상하지 않은 HEAD 또는 콘텐츠 변경은 `WORKSPACE_DRIFT`,
repository, worktree 또는 브랜치 변경은 `BINDING_MISMATCH`로 차단한다.

- 사용자가 workspace를 기존 checkpoint와 정확히 같게 복구하면 원래 state로
  재개한다.
- 같은 repository, worktree와 브랜치 안에서 복구할 수 없으면 사용자가 checkpoint
  재기준 설정을 명시적으로 승인할 수 있다.
- 재기준 설정은 새 HEAD와 fingerprint를 기록하고 `PLANNING`으로 돌아가 범위,
  전제, acceptance와 계획을 다시 승인받는다.
- repository, worktree 또는 브랜치가 다르면 재기준 설정하지 않고 원래 binding으로
  복귀하거나 새 태스크를 만든다.
- SCV는 drift를 자동으로 무시하거나 새 기준선으로 승격하지 않는다.

Git은 mutation deny-list가 아니라 kernel의 read-only subcommand allowlist로
제한한다. 허용 후보는 인자를 제한한 `status`, `diff`, `rev-parse`, `symbolic-ref`,
`ls-files`, `check-ignore`, `cat-file`, `show`, `log`, `merge-base`, `worktree list`다.
그 밖의 Git command는 등록되지 않았으므로 실행할 수 없다. 이에 따라 branch와
worktree 수명주기뿐 아니라 `add`, `rm`, `commit`, `merge`, `rebase`, `stash`,
`switch`, `checkout`, `restore`, `reset`, `clean`, `cherry-pick`, `revert`, `am`,
`apply`, `tag`, `push`, `fetch`, `pull`, `remote`, `submodule`, `config`, `update-ref`도
실행하지 않는다.
...
- 구현 backend는 결속된 worktree 안에서도 승인된 plan path root와 controller가
  만든 per-operation scratch만 쓰도록 제한한다. `.git`, runtime control state와
  setup 관리 링크는 kernel만 변경하고 backend 쓰기를 거부한다.
...
- 명령별 scratch는 macOS system temporary 영역에 `0700` 권한으로 만들고,
  worktree와 `.scv` control-plane 밖에 둔다. 쓰기는 승인된 plan path root와 해당
  scratch에만 허용한다.
- 중첩 Codex 또는 Claude backend는 임시 agent home을 사용한다. API key 또는
  검증된 격리 auth helper만 controller에 전달하고 tool subprocess에서는 제거한다.
  사용자 OAuth/keychain 파일, rules, skills, plugins와 일반 설정은 복사하거나
  상속하지 않는다.
...
- 기존 Git common directory의 learning store와 기존 태스크는 v1에서 실행하지
  않는 read-only compatibility 대상으로 유지한다.
...
- 향후 학습 기능을 승계하면 controller-only 기록, secret redaction, hashed
  ...
  경계를 유지해야 한다.
- v1에서는 legacy store를 무기한 read-only로 retain한다. 새 공개 Skill 재도입,
  archive 또는 폐기는 별도 ADR과 major layout migration 없이는 수행하지 않는다.

### 기존 데이터 migration

- setup은 legacy Git common directory 상태와 Git으로 추적된 기존
  `.scv/tasks/**`, `.scv/runtime/**`와 legacy manifest를 읽기 전용으로 탐지하고
  새 layout으로 오인해 덮어쓰지 않는다. tracked conflict가 있으면 쓰기 operation을
  차단하고 SCV가 자동 untrack, 이동 또는 삭제하지 않는다.
- legacy 태스크는 별도 승인된 export로 승인 문서만
  `.scv/docs/work-items`에 복사할 수 있으며 mutable 실행 상태를 재개하지 않는다.
...
10. clean slate에서 prompt rendering, backend dispatch, acceptance sandbox, 실행
    index, 수렴 판정과 evidence sealing을 서로 독립된 커널 모듈로 구현한다.
...
16. legacy compatibility fixture, 사용자 문서와 테스트를 새 동작으로 전환한다.
...
7. 계획 대화 중단과 재개 후에도 확정된 결정을 다시 묻지 않고 다음 미해결
   질문부터 계속한다.
8. 마지막 checkpoint 기반 재개, 예상 drift 차단, 정확 복구와 명시적 재기준 설정 후
   재승인이 모두 검증된다.
...
| 항목 | 상태 | 결정 문서 |
|---|---|---|
| bootstrap 배포와 source/target identity | Resolved | [D001](decisions/D001-package-activation.md) |
| 공통 Skill frontmatter와 linked/installed cutover | Resolved | [D001](decisions/D001-package-activation.md) |
| 호스트 최소 버전, invocation flag와 capability probe | Resolved | [D002](decisions/D002-host-contract.md) |
| Claude 검증 보조와 Codex 최종 리뷰 권한 | Resolved | [D002](decisions/D002-host-contract.md) |
| binding, checkpoint와 `BLOCKED` safe resume | Resolved | [D003](decisions/D003-state-workspace.md) |
| provider egress, sandbox, 환경과 인증 전달 | Resolved | [D004](decisions/D004-execution-isolation.md) |
| Ollama 정량 기준과 fallback | Resolved | [D005](decisions/D005-quality-lifecycle.md) |
| ADR metadata와 supersession | Resolved | [D005](decisions/D005-quality-lifecycle.md) |
| finding `must_fix`와 waiver | Resolved | [D005](decisions/D005-quality-lifecycle.md) |
| legacy learning과 runtime retention | Resolved | [D005](decisions/D005-quality-lifecycle.md) |
